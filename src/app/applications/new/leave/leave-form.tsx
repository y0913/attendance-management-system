...
export function LeaveForm({
  ...
  const [dayUnit, setDayUnit] = useState<DayUnit>('full');
  const [startDate, setStartDate] = useState(defaultStart);
  const [endDate, setEndDate] = useState(defaultEnd);
  const [reason, setReason] = useState('');

  const effectiveEnd = dayUnit === 'half' ? startDate : endDate;

  const days = useMemo(() => {
    if (dayUnit === 'half') {
      if (!isValidDate(startDate)) return 0;
      return isBusinessDay(startDate) ? 0.5 : 0;
    }
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    if (dayUnit === 'full' && !isValidDate(endDate)) {
      setError('日付を正しく入力してください');
      return;
    }
    if (dayUnit === 'full' && endDate < startDate) {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        ...
        setError(result.error.message ?? '申請に失敗しました');
      } else {
        setError('申請に失敗しました');
      }
    });
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex gap-3 text-sm">
          <label className="flex items-center gap-2">
            <input
              ...
              name="dayUnit"
              value="full"
              checked={dayUnit === 'full'}
              onChange={() => setDayUnit('full')}
            />
            全日
          </label>
          <label className="flex items-center gap-2">
            <input
              ...
              name="dayUnit"
              value="half"
              checked={dayUnit === 'half'}
              onChange={() => setDayUnit('half')}
            />
            半日（0.5 日）
          </label>
        </div>
      ...
      {dayUnit === 'full' ? (
        ...
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="lr-start">取得日</Label>
          <input
            id="lr-start"
            type="date"
            value={startDate}
